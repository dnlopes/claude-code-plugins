...
Skill-bearing plugins ship an isolated OpenCode package: `<plugin>/package.json` (`@dnlopes/<plugin>`) and `<plugin>/.opencode/plugin.js`. The adapter registers that plugin's `skills/`, and when present maps `agents/`, `commands/`, `.mcp.json`, and selected hooks. No shared monorepo OpenCode entrypoint — users install individual plugin directories. On version bumps (T2), also bump `<plugin>/package.json` to the same version and keep `files` listing `agents`/`commands`/`hooks`/`scripts`/`.mcp.json` when those exist. Plugins without skills (e.g. `status-line`) omit the OpenCode package. See [OpenCode](opencode.md).
