...
- **You MUST load the `docs-manager:documenting-repositories` skill first** for format standards, abstraction rules, and frontmatter spec.
...
User invokes via `/docs-manager:onboarding-repository [--auto]`.
...
Launch the `docs-manager:codebase-explorer` agent via the Task tool:
